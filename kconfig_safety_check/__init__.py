...
def add_kconfig_checks(l, arch, envi, kernel_version_num):
    ...
    # ELISA safety check
    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.13")):
    ...
    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.14")):
        l += [KconfigCheck('Memory management:Heap:Use after free', 'ELISA_safety, Intel', 'SLAB_FREELIST_HARDENED', 'y')]

    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("5.2")):
    ...
    if(envi in ('dev', 'debug')):
        ...
        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("5.3")):
    ...
    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.11")):
    ...
    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.15")):
        l += [KconfigCheck('Kernel Memory:Isolation of critical code', 'ELISA_safety', 'BPF_JIT_ALWAYS_ON', 'y')]

    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("5.13")):
        l += [KconfigCheck('Kernel Memory:Isolation of critical code', 'ELISA_safety', 'BPF_UNPRIV_DEFAULT_OFF', 'y')]

    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.18")):
    ...
    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("3.18")):
        l += [KconfigCheck('Stack memory:Stack overflow', 'ELISA_safety, Intel', 'SCHED_STACK_END_CHECK', 'y')]

    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.9")):
    ...
    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.20")):
        l += [KconfigCheck('Stack memory:Stack overflow', 'ELISA_safety', 'GCC_PLUGIN_STACKLEAK', 'y')]

    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.6")):
    ...
    if envi in ('dev', 'debug'):
        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.19")):
            l += [KconfigCheck('Heap memory:Heap overflow:Debug', 'ELISA_safety, Intel', 'DEBUG_LIST', 'y')]

    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.24")):
        l += [KconfigCheck('Driver: Heap overflow', 'ELISA_safety', 'DEBUG_SG', 'y')]

    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.10")):
    ...
    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.0")):
    ...
    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("3.0")):
    ...
    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.23")):
        l += [KconfigCheck('Kernel Memory:Kernel corruption of user space memory', 'ELISA_safety', 'HIBERNATION', 'n')]

    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.16")):
        l += [KconfigCheck('Kernel Memory:Kernel corruption of user space memory', 'ELISA_safety', 'KEXEC', 'n')]

    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.8")):
    ...
    if envi in ('dev', 'debug'):
        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.37")):
            l += [KconfigCheck('Kernel Memory:Debug', 'ELISA_safety', 'SECURITY_DMESG_RESTRICT', 'y')]

    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.25")):
        l += [KconfigCheck('Heap memory:R/W access to memory allocated to another software element', 'ELISA_safety', 'COMPAT_BRK', 'n')]

    if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.3")):
    ...
    if arch in ('ARM', 'ARM64'):
        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.16")):
            l += [KconfigCheck('Kernel Memory:W+X access to memory pages', 'ELISA_safety', 'DEBUG_WX', 'y')]
    if arch in ('X86_64', 'X86_32'):
        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.4")):
            l += [KconfigCheck('Kernel Memory:W+X access to memory pages', 'ELISA_safety', 'DEBUG_WX', 'y')]

    if envi in ('dev', 'trace'):
        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.18")):
            l += [KconfigCheck('Stack memory:Enable traceability', 'ELISA_safety', 'STACKTRACE_SUPPORT', 'y')]

        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.28")):
            l += [KconfigCheck('Stack memory:Enable traceability', 'ELISA_safety', 'STACK_TRACER', 'y')]

        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.9")):
            l += [KconfigCheck('Stack memory:Enable traceability', 'ELISA_safety', 'KPROBES', 'n')]

        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.27")):
        ...
        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.30")):
            l += [KconfigCheck('Stack memory:Enable traceability', 'ELISA_safety', 'FTRACE_SYSCALLS', 'y')]

        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.5.45")):
    ...
    if envi in ('dev', 'debug'):
        ...
        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("4.5")):
            l += [KconfigCheck('Debug:Kernel mode lockup', 'ELISA_safety', 'WQ_WATCHDOG', 'y')]

        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("5.8")):
        ...
        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.29")):
        ...
        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("3.11")):
        ...
        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("3.15")):
        ...
        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("2.6.15")):
            l += [KconfigCheck('Debug:RT mutexes', 'ELISA_safety', 'RCU_TORTURE_TEST', 'y')]

        if(parse_version(kernel_version_num) <= parse_version("5.17") and parse_version(kernel_version_num) >= parse_version("3.1")):
